...
const ServiceCatalogManagementPage = ({ handleBackClick }) => {
  ...
  const itemTypeOptions = [
    { value: '', label: 'Tất cả' },
    { value: 'THIRD_PARTY_PART', label: 'Phụ tùng bên thứ 3' },
  ...
  const calculateEstimate = async () => {
    if (estimateData.serviceItems.length === 0) {
  ...
  }, [activeTab, editingService, fetchAvailableServices]);
                        ...
                        {selectedServiceItem.currentPrice
                          ? (
                              <>
                                ...
                                {selectedServiceItem.priceRegion && (
                                  <span style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginLeft: '0.5rem' }}>
                                    (Khu vực: {selectedServiceItem.priceRegion})
                                  </span>
                                )}
                              </>
                            )
                          ...
                          <tr key={price.id}>
                            <td>
                              <span className={`service-catalog-type-badge ${price.itemType?.toLowerCase()}`}>
                                {price.itemType === 'THIRD_PARTY_PART' ? 'Phụ tùng bên thứ 3' : 'Dịch vụ'}
                              </span>
                            </td>
        ...
        >
          <div className="estimate-section">
            ...
            <button
              ...
              disabled={estimateLoading || estimateData.serviceItems.length === 0}
            >
              ...
              <div className="estimate-result">
                ...
                <div className="result-breakdown">
                  ...
                  <div className="result-grand-total">
                    <strong>Tổng cộng: {formatCurrency(estimateResult.totalAmount || estimateResult.totalServicesAmount, estimateResult.currency)}</strong>
                  </div>
                </div>
              </div>
            )}
