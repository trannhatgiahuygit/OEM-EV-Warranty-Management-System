      ...
      try {
        ...
        if (Array.isArray(usersResponse.data)) {
          ...
          results = usersResponse.data
            .filter(tech => {
              // First filter by service center if user is SC_STAFF
              if (user?.role === 'SC_STAFF' && userServiceCenterId && tech.serviceCenterId !== userServiceCenterId) {
                return false;
              }
              // Then filter by search query
              return String(tech.id).includes(techQuery) ||
