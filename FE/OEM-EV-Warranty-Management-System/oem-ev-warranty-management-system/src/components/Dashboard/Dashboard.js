...
import ServiceCenterManagementPage from './ServiceCenterManagementPage/ServiceCenterManagementPage';
...
const Dashboard = () => {
  ...
  // Update scroll indicators when sidebar content changes
  useEffect(() => {
    const element = sidebarContentRef.current;
    if (!element) return;

    // Handle sidebar scroll detection for fade indicators
    const handleSidebarScroll = () => {
      if (!element) return;

      const { scrollTop, scrollHeight, clientHeight } = element;
      const canScrollTop = scrollTop > 10;
      const canScrollBottom = scrollTop + clientHeight < scrollHeight - 10;

      setSidebarScrollState({ canScrollTop, canScrollBottom });
    };
    ...
    return () => {
      ...
      element.removeEventListener('scroll', handleSidebarScroll);
    ...
    <>
      ...
      <div className="dashboard-page">
        <div className="dashboard-container">
          <motion.div
            ref={sidebarRef}
            className={`dashboard-sidebar ${sidebarScrollState.canScrollTop ? 'scrollable-top' : ''} ${sidebarScrollState.canScrollBottom ? 'scrollable-bottom' : ''}`}
            initial="hidden"
            animate="visible"
            variants={{
              visible: {
